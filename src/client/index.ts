...
function copyButton(value: string): string { return `<button type="button" data-copy="${html(value)}" style="margin-left:8px;padding:2px 6px;border:1px solid var(--dsw-alias-border-l2);border-radius:4px;background:transparent;color:inherit;font:inherit;font-size:12px;cursor:pointer">复制</button>` }
...
export function apply(ctx: any): void {
  ...
  const showTouchItems = async () => {
    ...
    const body = view.querySelector<HTMLElement>('.pc-body')!
    body.innerHTML = `<div class="pc-toolbar"><strong>今日微信触达跟进</strong><span class="pc-note">${page.content.length} 条</span></div>${page.content.map(item => { const contact = item.contactValue || item.wechatId || '-'; const command = bindCommand(item); return `<article class="pc-touch" data-touch-id="${item.id}"><b>${html(item.actualCompanyName || item.sourceName || '未关联公司')}</b><div class="pc-touch-grid"><div class="pc-field"><label>联系方式</label><span>${html(contact)}${copyButton(contact)}</span></div><div class="pc-field"><label>群名</label><span>${html(groupName(item))}</span></div><div class="pc-field"><label>口令</label><span>${html(command)}${copyButton(command)}</span></div><div class="pc-field"><label>微信搜索</label><select data-field="wechatExists"><option value="" ${item.wechatExists == null ? 'selected' : ''}>未填写</option><option value="1" ${item.wechatExists === 1 ? 'selected' : ''}>已搜到</option><option value="0" ${item.wechatExists === 0 ? 'selected' : ''}>未搜到</option></select></div><div class="pc-field"><label>用户昵称</label><input data-field="wechatNickname" value="${html(item.wechatNickname)}" placeholder="填写微信昵称"></div><button class="pc-save" type="button">保存</button></div></article>` }).join('') || '<p class="pc-empty">今日暂无微信触达跟进。</p>'}`
