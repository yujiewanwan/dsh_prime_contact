...
async function get<T>(path: string): Promise<T> { const response = await fetch(`${API}${path}`); if (!response.ok) throw new Error(response.status === 503 ? '请在设置 > 插件 > 插件配置中完成 Prime Contact 登录。' : 'Prime Contact 请求失败。'); const body = await response.json() as { data?: T }; return (body.data ?? body) as T }
